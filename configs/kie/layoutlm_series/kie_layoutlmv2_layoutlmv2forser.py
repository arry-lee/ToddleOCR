...
from torch.nn import LayoutLMv2ForSer
...
from ppocr.postprocess.vqa_token_ser_layoutlm_postprocess import VQASerTokenLayoutLMPostProcess
from ppocr.data.simple_dataset import SimpleDataSet
from ppocr.data.imaug.operators import DecodeImage, KeepKeys, ToCHWImage, NormalizeImage, Resize
...
class Model(ConfigModel):
    ...
    Transform = None
    Backbone = _(LayoutLMv2ForSer, pretrained=True, checkpoints=None, num_classes=7)
    ...
    class Train:
        Dataset = _(SimpleDataSet, data_dir="train_data/XFUND/zh_train/image", label_file_list=['train_data/XFUND/zh_train/train.json'])
        transforms = _[DecodeImage(img_mode="RGB", channel_first=False), VQATokenLabelEncode(contains_re=False, algorithm="LayoutLMv2", class_path="train_data/XFUND/class_list_xfun.txt"), VQATokenPad(max_seq_len=512, return_attention_mask=True), VQASerTokenChunk(max_seq_len=512), Resize(size=[224, 224]), NormalizeImage(scale=1, mean=[123.675, 116.28, 103.53], std=[58.395, 57.12, 57.375], order="hwc"), ToCHWImage(), KeepKeys(keep_keys=['input_ids', 'bbox', 'attention_mask', 'token_type_ids', 'image', 'labels'])]
