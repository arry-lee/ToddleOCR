...
from ppocr.losses.combined_loss import CombinedLoss
...
from ppocr.data.imaug.rec_img_aug import RecAug, RecResizeImg
...
class Model(ConfigModel):
    ...
    loss = CombinedLoss(
        loss_config_list=[
            ...
            {"CenterLoss": {"weight": 0.05, "num_classes": 6625, "feat_dim": 96, "center_file_path": None}},
        ]
    )
    metric = RecMetric(main_indicator="acc")
    Optimizer = _(Adam, betas=[0.9, 0.999])
    LRScheduler = _(Piecewise, decay_epochs=[700], values=[0.001, 0.0001], warmup_epoch=5)
    PostProcessor = _(
        CTCLabelDecode,
    )
