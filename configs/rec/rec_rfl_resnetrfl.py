...
from ppocr.modeling.transforms.tps import TPS
from ppocr.modeling.backbones.rec_resnet_rfl import ResNetRFL
from ppocr.modeling.necks.rf_adaptor import RFAdaptor
from ppocr.modeling.heads.rec_rfl_head import RFLHead
from ppocr.losses.rec_rfl_loss import RFLLoss
from ppocr.metrics.rec_metric import CNTMetric
from torch.optim import AdamW
from torch.nn import Piecewise
from ppocr.postprocess.rec_postprocess import RFLLabelDecode
from ppocr.data.lmdb_dataset import LMDBDataSet
from ppocr.data.imaug.operators import KeepKeys, DecodeImage
from ppocr.data.imaug.label_ops import RFLLabelEncode
from ppocr.data.imaug.rec_img_aug import RFLRecResizeImg
...
class Model(ConfigModel):
    ...
    Head = _(
        RFLHead, in_channels=512, hidden_size=256, batch_max_legnth=25, out_channels=38, use_cnt=True, use_seq=False
    )
    loss = RFLLoss()
    metric = CNTMetric(main_indicator="acc")
    Optimizer = _(AdamW, beta1=0.9, beta2=0.999, weight_decay=0.0, clip_norm_global=5.0)
    LRScheduler = _(Piecewise, decay_epochs=[3, 4, 5], values=[0.001, 0.0003, 9e-05, 2.7e-05])
    PostProcessor = _(
        RFLLabelDecode,
    )
    ...
    class Train:
        ...
        transforms = _[
            ...
            KeepKeys(keep_keys=["image", "label", "length", "cnt_label"]),
        ]
