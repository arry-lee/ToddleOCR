...
class SDMGRHead(nn.Module):
    ...
    def forward(self, input, targets):
        ...
        all_nodes = torch.concat(
            [
                torch.concat(
                    [text, torch.zeros((text.shape[0], max_num - text.shape[1]))], -1
                )
                for text in texts
            ]
        )
        ...
        temp_all_nums = torch.unsqueeze(temp_all_nums, 0).repeat(
        ...
        one_hot = torch.multiply(temp_all_nodes, one_hot.astype("float32")).sum(
            dim=1, keepdim=True
        )
    ...
    def forward(self, nodes, edges, nums):
        ...
        for num in nums:
            ...
            cat_nodes.append(
                torch.concat(
                    [
                        torch.unsqueeze(sample_nodes.unsqueeze(1), 0).repeat(
                            -1, num, -1
                        ),
                        torch.unsqueeze(sample_nodes.unsqueeze(0), 0).repeat(
                            num, -1, -1
                        ),
                    ],
                    -1,
