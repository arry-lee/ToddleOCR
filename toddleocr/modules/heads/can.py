...
class CountingDecoder(nn.Module):
    def __init__(self, in_channel, out_channel, kernel_size):
        ...
        self.pred_layer = nn.Sequential(
            nn.Conv2d(512, self.out_channel, kernel_size=1, bias=False), nn.Sigmoid()
        )
...
class AttDecoder(nn.Module):
    ...
    def init_hidden(self, features, feature_mask):
        average = torch.sum(
            torch.sum(features * feature_mask, dim=-1), dim=-1
        ) / torch.sum((torch.sum(feature_mask, dim=-1)), dim=-1)
...
class Attention(nn.Module):
    ...
    def forward(
        ...
        alpha_score = torch.tanh(
            torch.unsqueeze(query, [1, 2])
            + coverage_alpha
            + torch.transpose(cnn_features_trans, [0, 2, 3, 1])
        )
        ...
        alpha = energy_exp / (
            torch.unsqueeze(torch.sum(torch.sum(energy_exp, -1), -1), [1, 2]) + 1e-10
        )
