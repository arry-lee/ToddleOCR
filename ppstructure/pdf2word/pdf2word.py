    ...
    def ppocrPrecitor(self, imgs, img_name):
        ...
        if all_res != []:
            try:
                ...
                print("error in layout recovery image:{}, err msg: {}".format(img_name, ex))
...
class APP_Image2Doc(QWidget):
    ...
    def setupUi(self):
        ...
        self.startCNButton.clicked.connect(functools.partial(self.handleStartSignal, "CN", False))
        ...
        self.startENButton.clicked.connect(functools.partial(self.handleStartSignal, "EN", False))

        self.PDFParserButton = QPushButton("PDF解析", self)
        layout.addWidget(self.PDFParserButton, 0, 3, 1, 1)
        self.PDFParserButton.clicked.connect(functools.partial(self.handleStartSignal, "CN", True))
    ...
    def initPredictor(self, lang="EN"):
        ...
        if lang == "EN":
            args.det_model_dir = os.path.join(root, "inference", "en_PP-OCRv3_det_infer")  # 此处从这里找到模型存放位置
            ...
            args.det_model_dir = os.path.join(root, "inference", "cn_PP-OCRv3_det_infer")  # 此处从这里找到模型存放位置
    ...
    def handleOpenFileSignal(self):
        ...
        可以多选图像文件
        """
        selectedFiles = QFileDialog.getOpenFileNames(self, "多文件选择", "/", "图片文件 (*.png *.jpeg *.jpg *.bmp *.pdf)")[0]
