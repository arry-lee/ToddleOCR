...
class StructureSystem(object):
    def __init__(self, args):
        ...
        if args.image_orientation:
            ...
            self.image_orientation_predictor = paddleclas.PaddleClas(model_name="text_image_orientation")
        ...
        if self.mode == "structure":
            ...
            if args.layout == False and args.ocr == True:
                args.ocr = False
                logger.warning("When args.layout is false, args.ocr is automatically set to false")
            args.drop_score = 0
            # init model
            ...
            for region in layout_res:
                ...
                if region["label"] == "table":
                    if self.table_system is not None:
                        res, table_time_dict = self.table_system(roi_img, return_ocr_result_in_table)
                    ...
                    if self.text_system is not None:
                        if self.recovery:
                            ...
                            filter_boxes, filter_rec_res, ocr_time_dict = self.text_system(wht_im)
                        else:
                            filter_boxes, filter_rec_res, ocr_time_dict = self.text_system(roi_img)
...
def save_structure_res(res, save_folder, img_name, img_idx=0):
    ...
    res_cp = deepcopy(res)
    # save res
    with open(os.path.join(excel_save_folder, "res_{}.txt".format(img_idx)), "w", encoding="utf8") as f:
        for region in res_cp:
            roi_img = region.pop("img")
            f.write("{}\n".format(json.dumps(region)))

            if region["type"].lower() == "table" and len(region["res"]) > 0 and "html" in region["res"]:
                excel_path = os.path.join(excel_save_folder, "{}_{}.xlsx".format(region["bbox"], img_idx))
