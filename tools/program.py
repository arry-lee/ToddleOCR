...
def check_device(use_gpu, use_xpu=False, use_npu=False, use_mlu=False):
    ...
    try:
        ...
        if use_xpu and not torch.device.is_compiled_with_xpu():
            ...
            sys.exit(1)
        if use_npu:
            if int(torch.version.major) != 0 and int(torch.version.major) <= 2 and int(torch.version.minor) <= 4:
...
def train(
    ...
    for epoch in range(start_epoch, epoch_num + 1):
        ...
        for idx, batch in enumerate(train_dataloader):
            ...
            # use amp
            if scaler:
                with torch.amp.auto_cast(level=amp_level, custom_black_list=amp_custom_black_list):
            ...
            if cal_metric_during_train and epoch % calc_epoch_interval == 0:  # only rec and cls need
                ...
                if model_type in ["kie", "sr"]:
                    ...
                    if config["Loss"]["name"] in ["MultiLoss", "MultiLoss_v2"]:  # for multi head loss
                        post_result = post_process_class(preds["ctc"], batch[1])  # for CTC head out
...
def eval(model, valid_dataloader, post_process_class, eval_class, model_type=None, extra_input=False, scaler=None, amp_level="O2", amp_custom_black_list=[]):
    ...
    with torch.no_grad():
        total_frame = 0.0
        total_time = 0.0
        pbar = tqdm(total=len(valid_dataloader), desc="eval model:", position=0, leave=True)
...
def get_center(model, eval_dataloader, post_process_class):
    ...
    max_iter = len(eval_dataloader) - 1 if platform.system() == "Windows" else len(eval_dataloader)
    char_center = dict()
