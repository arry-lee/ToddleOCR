...
class RFLRecResizeImg(object):
    ...
    def __call__(self, data):
        ...
        norm_img, valid_ratio = resize_norm_img(img, self.image_shape, self.padding, self.interpolation)
        data["image"] = norm_img
        data["valid_ratio"] = valid_ratio
        return data
...
class PRENResizeImg(object):
    ...
    def __call__(self, data):
        img = data["image"]
        resized_img = cv2.resize(img, (self.dst_w, self.dst_h), interpolation=cv2.INTER_LINEAR)
...
def srn_other_inputs(image_shape, num_heads, max_text_length):
    ...
    gsrm_slf_attn_bias1 = np.triu(gsrm_attn_bias_data, 1).reshape([1, max_text_length, max_text_length])
    gsrm_slf_attn_bias1 = np.tile(gsrm_slf_attn_bias1, [num_heads, 1, 1]) * [-1e9]

    gsrm_slf_attn_bias2 = np.tril(gsrm_attn_bias_data, -1).reshape([1, max_text_length, max_text_length])
