...
class KieLabelEncode(object):
    ...
    def convert_canonical(self, points_x, points_y):
        ...
        points_to_lefttop = [LineString([points[i], Point(min_x, min_y)]) for i in range(4)]
...
class SEEDLabelEncode(BaseRecLabelEncode):
    ...
    def __call__(self, data):
        ...
        text = text + [len(self.character) - 3] + [len(self.character) - 2] * (self.max_text_len - len(text) - 1)
        data["label"] = np.array(text)
        return data
    ...
    def __init__(
        ...
        **kwargs
        ...
        tokenizer_dict = {
            ...
            "LayoutLM": {"class": LayoutLMTokenizer, "pretrained_model": "layoutlm-base-uncased"},
            "LayoutLMv2": {"class": LayoutLMv2Tokenizer, "pretrained_model": "layoutlmv2-base-uncased"},
        }
        self.contains_re = contains_re
        tokenizer_config = tokenizer_dict[algorithm]
        self.tokenizer = tokenizer_config["class"].from_pretrained(tokenizer_config["pretrained_model"])
