...
class TwoStepCosineLR(LRScheduler):
    def __init__(self, optimizer, T_max1, T_max2, eta_min=0, last_epoch=-1, verbose=False):
        if not isinstance(T_max1, int):
            raise TypeError("The type of 'T_max1' must be 'int', but received %s." % type(T_max1))
        if not isinstance(T_max2, int):
            raise TypeError("The type of 'T_max2' must be 'int', but received %s." % type(T_max2))
        if not isinstance(eta_min, (float, int)):
            raise TypeError("The type of 'eta_min' must be 'float' or 'int', but received %s." % type(eta_min))
    ...
    def get_lr(self):
        if self.last_epoch <= self.T_max1:
            if self.last_epoch == 0:
                ...
                return [
                    lr + (base_lr - self.eta_min) * (1 - math.cos(math.pi / self.T_max1)) / 2
            ...
            if (self.last_epoch - 1 - self.T_max2) % (2 * self.T_max2) == 0:
                return [
                    lr + (base_lr - self.eta_min) * (1 - math.cos(math.pi / self.T_max2)) / 2
                    for lr, base_lr in zip(self.get_last_lr(), self.base_lrs)
                ]
    ...
    def _get_closed_form_lr(self):
        if self.last_epoch <= self.T_max1:
            return [
                self.eta_min + (base_lr - self.eta_min) * (1 + math.cos(math.pi * self.last_epoch / self.T_max1)) / 2
