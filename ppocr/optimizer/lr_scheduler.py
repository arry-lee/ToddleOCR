...
class CyclicalCosineDecay(LRScheduler):
    def __init__(self, learning_rate, T_max, cycle=1, last_epoch=-1, eta_min=0.0, verbose=False):
        """
    ...
    def get_lr(self):
        ...
        lr = self.eta_min + 0.5 * (self.base_lr - self.eta_min) * (1 + math.cos(math.pi * reletive_epoch / self.cycle))
        return lr
