...
class FCELoss(nn.Module):
    ...
    def forward(self, preds, labels):
        ...
        loss_tr = torch.to_tensor(0.).astype('float32')
        loss_tcl = torch.to_tensor(0.).astype('float32')
        loss_reg_x = torch.to_tensor(0.).astype('float32')
        loss_reg_y = torch.to_tensor(0.).astype('float32')
        loss_all = torch.to_tensor(0.).astype('float32')
