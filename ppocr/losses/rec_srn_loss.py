...
class SRNLoss(nn.Module):
    ...
    def forward(self, predicts, batch):
        ...
        casted_label = torch.reshape(x=casted_label, shape=[-1, 1])
        ...
        cost_word = torch.reshape(x=torch.sum(cost_word), shape=[1])
        cost_gsrm = torch.reshape(x=torch.sum(cost_gsrm), shape=[1])
        cost_vsfd = torch.reshape(x=torch.sum(cost_vsfd), shape=[1])
