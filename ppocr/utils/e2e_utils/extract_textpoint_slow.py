...
def point_pair2poly(point_pair_list):
    ...
    pair_info = (pair_length_list.max(), pair_length_list.min(), pair_length_list.mean())
...
def generate_pivot_list_horizontal(p_score, p_char_maps, f_direction, score_thresh=0.5, is_backbone=False, image_id=0):
    ...
    instance_count, instance_label_map = cv2.connectedComponents(p_tcl_map_bi.astype(np.uint8), connectivity=8)
