...
__all__ = ['CNTMetric', 'CANMetric', 'RecMetric']
...
class CANMetric:
    ...
    def __call__(self, preds, batch, **kwargs):
        ...
        word_scores = [SequenceMatcher(None, s1[:int(np.sum(s3))], s2[:int(np.sum(s3))], autojunk=False).ratio() * (
                    len(s1[:int(np.sum(s3))]) + len(s2[:int(np.sum(s3))])) / len(s1[:int(np.sum(s3))]) / 2 for
                       (s1, s2, s3) in zip(word_label, word_pred, word_label_mask)]
