...
class MbConvBlock(nn.Module):
    def __init__(self, block_args):
        ...
        if self._block_args.expand_ratio != 1:
            self._expand_conv = nn.Conv2d(self.inp, oup, 1, bias=False)
        ...
        if isinstance(s, list):
            s = s[0]
        self._depthwise_conv = nn.Conv2d(oup, oup, groups=oup, kernel_size=k, stride=s, padding="same", bias=False)
...
class EfficientNetb3_PREN(nn.Module):
    def __init__(self, in_channels):
        ...
        self._conv_stem = nn.Conv2d(in_channels, out_channels, 3, 2, padding="same", bias=False)
        ...
        for i, block_args in enumerate(self._blocks_args):
            block_args = block_args._replace(
                input_filters=EffUtils.round_filters(block_args.input_filters, self._global_params),
