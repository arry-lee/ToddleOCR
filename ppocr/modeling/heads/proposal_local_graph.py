...
class ProposalLocalGraphs:
    ...
    def propose_comps_and_attribs(self, text_region_map, center_region_map, top_height_map, bot_height_map, sin_map, cos_map):
        ...
        for text_comp_box in text_comp_boxes:
            ...
            temp_region_patch = text_region_map[min_coord[1] : (max_coord[1] + 1), min_coord[0] : (max_coord[0] + 1)]
            score = cv2.mean(temp_region_patch, temp_comp_mask)[0]
            scores.append(score)
        scores = np.array(scores).reshape((-1, 1))
        text_comps = np.hstack([text_comps[:, :-1], scores])

        h = top_height_map[y, x].reshape((-1, 1)) + bot_height_map[y, x].reshape((-1, 1))
    ...
    def generate_local_graphs(self, sorted_dist_inds, node_feats):
        ...
        assert sorted_dist_inds.shape[0] == sorted_dist_inds.shape[1] == node_feats.shape[0]
        ...
        num_max_nodes = max([len(pivot_local_graph) for pivot_local_graph in pivot_local_graphs])
        ...
        return (local_graphs_node_feat, adjacent_matrices, pivots_knn_inds, pivots_local_graphs)
