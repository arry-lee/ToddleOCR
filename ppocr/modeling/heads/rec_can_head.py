    ...
    def forward(self, cnn_features, labels, counting_preds, images_mask):
        ...
        for i in range(num_steps):
            ...
            if self.is_train:
                ...
                word = torch.multiply(word, labels[:, i])  # labels are oneslike tensor in infer/predict mode

        return word_probs

    def init_hidden(self, features, feature_mask):
        average = torch.sum(torch.sum(features * feature_mask, axis=-1), dim=-1) / torch.sum((torch.sum(feature_mask, dim=-1)), dim=-1)
...
class Attention(nn.Module):
    ...
    def forward(self, cnn_features, cnn_features_trans, hidden, alpha_sum, image_mask=None):
        ...
        alpha = energy_exp / (torch.unsqueeze(torch.sum(torch.sum(energy_exp, -1), -1), [1, 2]) + 1e-10)
...
class CANHead(nn.Module):
    def __init__(self, in_channel, out_channel, ratio, attdecoder, **kwargs):
        ...
        self.counting_decoder1 = CountingDecoder(self.in_channel, self.out_channel, 3)  # mscm
