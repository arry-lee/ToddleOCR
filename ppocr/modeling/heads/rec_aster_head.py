...
class AsterHead(nn.Module):
    def __init__(self, in_channels, out_channels, sDim, attDim, max_len_labels, time_step=25, beam_width=5, **kwargs):
        ...
        self.decoder = AttentionRecognitionHead(in_channels, out_channels, sDim, attDim, max_len_labels)
    ...
    def forward(self, x, targets=None, embed=None):
        ...
        if self.training:
            ...
            rec_pred, rec_pred_scores = self.decoder.beam_search(x, self.beam_width, self.eos, embedding_vectors)
            return_dict["rec_pred"] = rec_pred
...
class AttentionRecognitionHead(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, sDim, attDim, max_len_labels):
        ...
        self.decoder = DecoderUnit(sDim=sDim, xDim=in_channels, yDim=self.num_classes, attDim=attDim)
    ...
    def beam_search(self, x, beam_width, eos, embed):
        ...
        re_sorted_idx = torch.reshape(re_sorted_idx + pos_index.expand_as(re_sorted_idx), [batch_size * beam_width])
