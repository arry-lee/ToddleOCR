...
class SVTRNet(nn.Module):  # todo 太多了
    def __init__(
        self,
        img_size=(32, 100),
        in_channels=3,
    ...
    def forward_features(self, x):
        ...
        if self.patch_merging is not None:
            x = self.sub_sample1(x.transpose([0, 2, 1]).reshape([0, self.embed_dim[0], self.HW[0], self.HW[1]]))
        for blk in self.blocks2:
            x = blk(x)
        if self.patch_merging is not None:
            x = self.sub_sample2(x.transpose([0, 2, 1]).reshape([0, self.embed_dim[1], self.HW[0] // 2, self.HW[1]]))
    ...
    def forward(self, x):
        ...
        if self.last_stage:
            if self.patch_merging is not None:
                ...
                h = self.HW[0]
            x = self.avg_pool(x.transpose([0, 2, 1]).reshape([0, self.embed_dim[2], h, self.HW[1]]))
            x = self.last_conv(x)
            x = self.hardswish(x)
            x = self.dropout(x)
        if self.use_lenhead:
