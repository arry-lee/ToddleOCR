...
class ResNetRFL(nn.Module):
    def __init__(
        self, in_channels, out_channels=512, use_cnt=True, use_seq=True
    ):
        """
...
class ResNetBase(nn.Module):
    def __init__(self, in_channels, out_channels, block, layers):
        ...
        self.layer1 = self._make_layer(
            block, self.out_channels_block[0], layers[0]
        )
...
class RFLBase(nn.Module):
    ...
    def __init__(self, in_channels, out_channels=512):
        super().__init__()
        self.ConvNet = ResNetBase(
            in_channels, out_channels, BasicBlock, [1, 2, 5, 3]
        )
