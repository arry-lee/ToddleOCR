...
class CenterLoss(nn.Module):
    ...
    def forward(self, predicts, batch=None):
        ...
        # Reshape特征向量为2D
        feats_reshape = torch.reshape(features, [-1, features.shape[-1]]).type(torch.float64)
        ...
        square_center = square_center.expand(self.num_classes, batch_size).type(torch.float64)
        ...
        mask = torch.eq(classes.unsqueeze(0).expand(batch_size, self.num_classes), label).type(torch.float64)
