...
class E2EMetric:
    ...
    def __call__(self, preds, batch, **kwargs):
        if self.mode == 'A':
            ...
            for (pred, gt_polyons, gt_strs, ignore_tags) in zip([preds], gt_polyons_batch, [gt_strs_batch],
                                                                ignore_tags_batch):
                gt_info_list = [{'points': gt_polyon, 'text': gt_str, 'ignore': ignore_tag} for
                                (gt_polyon, gt_str, ignore_tag) in zip(gt_polyons, gt_strs, ignore_tags)]
                e2e_info_list = [{'points': det_polyon, 'texts': pred_str} for (det_polyon, pred_str) in
