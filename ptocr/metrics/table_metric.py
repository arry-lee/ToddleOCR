...
class TableStructureMetric:
    ...
    def __init__(self, main_indicator='acc', eps=1e-06, del_thead_tbody=False, **kwargs):
    ...
    def __call__(self, pred_label, batch=None, *args, **kwargs):
        ...
        for ((pred, pred_conf), target) in zip(pred_structure_batch_list, gt_structure_batch_list):
            ...
            if self.del_thead_tbody:
                pred_str = pred_str.replace('<thead>', '').replace('</thead>', '').replace('<tbody>', '').replace(
                    '</tbody>', '')
                target_str = target_str.replace('<thead>', '').replace('</thead>', '').replace('<tbody>', '').replace(
                    '</tbody>', '')
...
class TableMetric:
    ...
    def __init__(self, main_indicator='acc', compute_bbox_metric=False, box_format='xyxy', del_thead_tbody=False,
                 **kwargs):
    ...
    def prepare_bbox_metric_input(self, pred_label):
        ...
        for batch_idx in range(batch_num):
            pred_bbox_list = [self.format_box(pred_box) for pred_box in preds['bbox_batch_list'][batch_idx]]
        ...
        return [pred_bbox_batch_list, [0, 0, gt_bbox_batch_list, gt_ignore_tags_batch_list]]
    ...
    def get_metric(self):
        ...
        if self.main_indicator == self.bbox_metric.main_indicator:
            ...
            for sub_key in structure_metric:
                output['structure_metric_{}'.format(sub_key)] = structure_metric[sub_key]
        else:
