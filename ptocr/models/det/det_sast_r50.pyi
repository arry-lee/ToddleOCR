...
class Model(ConfigModel):
    ...
    postprocessor = SASTPostProcess(score_thresh=0.5, sample_pts_num=6, nms_thresh=0.2, expand_scale=1.2, shrink_ratio_of_width=0.2)
    Optimizer = _(Adam,betas=[0.9, 0.999], lr=0.001)
    LRScheduler = _(ConstantLR,)
    class Data:
        dataset = SimpleDataSet
        root = "train_data/"
        label_file_list:"total_text_icdar_14pt/test_label_json.txt" = ['art_latin_icdar_14pt/train_no_tt_test/train_label_json.txt', 'total_text_icdar_14pt/train_label_json.txt']
    class Loader:
        shuffle:False = True
        drop_last = False
        batch_size:1 = 4
        num_workers:2 = 4
    Transforms = _[DecodeImage(img_mode="BGR", channel_first=False),DetLabelEncode():...,SASTProcessTrain(image_shape=[512, 512], min_crop_side_ratio=0.3, min_crop_size=24, min_text_size=4, max_text_size=512):,:DetResizeForTest(resize_long=768),:NormalizeImage(scale="1./255.", mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225], order="hwc"),:ToCHWImage(),KeepKeys("image","score_map","border_map","training_mask","tvo_map","tco_map"):KeepKeys("image","shape","polys","ignore_tags"):...]