...
def transmodel():
    ...
    paddle_params = paddle.load(
        r'C:\Users\dell\.toddlewhl\rec\ch\ch_PP-OCRv2_rec_infer\inference'
    )
    ...
    for k, v in x.items():
        ...
        if kk.endswith('num_batches_tracked'):
            if kk in paddle_params:
                print('bn',kk)
                d[k] = p2t(paddle_params[kk])
            else:
                print('bn', kk)
                d[k] = torch.tensor(0) #去掉
        ...
        if "fc" in k:
            print('fc',k,d[k].shape)
            d[k] = d[k].T
    print('saving')
    torch.save(d, "D:/dev/.model/paddocr/inference.pth")
