...
from ptocr.modules.backbones.resnet.rec_resnet_rfl import ResNetRFL
...
class Model(ConfigModel):
    ...
    Optimizer = _(
        AdamW, beta1=0.9, beta2=0.999, weight_decay=0.0, clip_norm_global=5.0
    )
    LRScheduler = _(
        PiecewiseLR,
        decay_epochs=[3, 4, 5],
    ...
    class Train:
        ...
        transforms = _[
            ...
            RFLLabelEncode(),
            RFLRecResizeImg(
                image_shape=[1, 32, 100], padding=False, interpolation=2
            ),
            KeepKeys(keep_keys=["image", "label", "length", "cnt_label"]),
        ]
        DATALOADER = _(
            shuffle=True, batch_size=64, drop_last=True, num_workers=8
        )
