...
class Model(ConfigModel):
    ...
    postprocessor = CTCLabelDecode(character_dict_path=character_dict_path)

    Transform = None
    Backbone = _(ResNet_Rec_Vd, layers=34)
    Neck = _(SequenceEncoder, encoder_type="rnn", hidden_size=256)
    Head = _(CTCHead,out_channels=len(postprocessor.character)+1, fc_decay=4e-05)
    ...
    class Train:
        ...
        transforms = _[DecodeImage(img_mode="BGR", channel_first=False), RecAug(), CTCLabelEncode(max_text_length=25,character_dict_path="ptocr/utils/ppocr_keys_v1.txt"), RecResizeImg(image_shape=[3, 32, 320]), KeepKeys(keep_keys=['image', 'label', 'length'])]
    ...
    class Eval:
        ...
        transforms = _[DecodeImage(img_mode="BGR", channel_first=False), CTCLabelEncode(max_text_length=25,character_dict_path="ptocr/utils/ppocr_keys_v1.txt"), RecResizeImg(image_shape=[3, 32, 320]), KeepKeys(keep_keys=['image', 'label', 'length'])]
        DATALOADER = _(shuffle=False, drop_last=False, batch_size=256, num_workers=8)
    class Infer:
        transforms = _[DecodeImage(img_mode="BGR", channel_first=False), RecResizeImg(image_shape=[3, 32, 320]), KeepKeys(keep_keys=['image'])]
...
def transmodel():
    ...
    # print(t_keys)
    paddle_params = paddle.load(r'D:\dev\github\ToddleOCR\model\ch_ppocr_server_v2.0_rec_pre\best_accuracy.pdparams')
    ...
    maps = {"bn.running_mean":"_batch_norm._mean",
    ...
    torch.save(d,'model/ch_ppocr_server_v2.0_rec_pre/best_accuracy.pth')
...
if __name__ == '__main__':
    # transmodel()
    m = Model()
    m.rec("D:\dev\github\ToddleOCR\doc\imgs_words\ch\word_4.jpg",
          r"D:\dev\github\ToddleOCR\model\ch_ppocr_server_v2.0_rec_pre\best_accuracy.pth")
