...
from ptocr.modules.heads.table_master import TableMasterHead
...
from ptocr.transforms.label_ops import TableBoxEncode, TableMasterLabelEncode
...
class Model(ConfigModel):
    ...
    metric = TableMetric(
        main_indicator="acc", compute_bbox_metric=False, box_format="xywh"
    )
    Optimizer = _(Adam, betas=[0.9, 0.999], lr=0.001)
    LRScheduler = _(
        MultiStepLR, milestones=[12, 15], gamma=0.1, warmup_epoch=0.02
    )
    PostProcessor = _(
        TableMasterLabelDecode, box_shape="pad", merge_no_span_structure=True
    )
    ...
    class Train:
        Dataset = _(
            ...
            label_file_list=[
                "train_data/table/pubtabnet/PubTabNet_2.0.0_train.jsonl"
            ],
        )
        transforms = _[
            ...
            ResizeTableImage(max_len=480, resize_bboxes=True),
            PaddingTableImage(size=[480, 480]),
            TableBoxEncode(in_box_format="xywh", out_box_format="xywh"),
            NormalizeImage(
                scale="1./255.",
                mean=[0.5, 0.5, 0.5],
                std=[0.5, 0.5, 0.5],
        ...
        DATALOADER = _(
            shuffle=True, batch_size=10, drop_last=True, num_workers=8
        )
    ...
    class Eval:
        ...
        DATALOADER = _(
            shuffle=False, drop_last=False, batch_size=10, num_workers=8
        )
