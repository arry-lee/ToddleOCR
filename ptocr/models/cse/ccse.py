...
from torchvision.models.detection.backbone_utils import (
    ...
    _validate_trainable_layers, resnet_fpn_backbone,
)
...
def coco_target_transforms(target):
    ...
    for ann in target:
        ...
        if mask.shape != (120, 120):
            mask = torch.nn.functional.interpolate(mask.float(), size=(120, 120)).squeeze(0).byte()  # 插值并移除额外的维度
        bbox = torch.tensor(
            [
                ann["bbox"][0] / 120,
                ann["bbox"][1] / 120,
...
class Model(ConfigModel):
    ...
    # 主干网络
    Backbone = _(resnet_fpn_backbone, 'resnet50', weights=ResNet50_Weights.DEFAULT, trainable_layers=5) #'features'
    # 颈部
    Neck = _(_RegionProposalNetwork,)
    # 头部
...
if __name__ == "__main__":
    ...
    for i in n:
        ...
        for box in d['head_out'][0][0]['boxes']:
            print(box)
            draw.rectangle((int(box[0] * 120), int(box[1] * 120), int(box[2] * 120), int(box[3] * 120)),
                outline='green')
