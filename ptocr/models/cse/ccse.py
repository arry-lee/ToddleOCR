...
#     def forward(self,x,target=None,other=None):
#         ...
#         'process sth'
#         out = x
#         return out
...
#     ...
#
# package = dict(x='a',target='b',other='c')
# package = wraper(DictModule)()(package)
...
from ptocr.datasets.wrap import dict_module
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.backbone = _resnet_fpn_extractor(
            self.backbone, trainable_backbone_layers
        )
...
# class ResNet50()
#
# def _RegionProposalNetwork(in_channels,
...
#         rpn_anchor_generator := AnchorGenerator(anchor_sizes, aspect_ratios * len(anchor_sizes)),
...
class _RoIHeads(RoIHeads):
    def __init__(
        ...
        if box_head is None:
            ...
            box_head = TwoMLPHead(
                in_channels * resolution**2, representation_size
            )
    ...
    def __call__(self, data):
        ...
        targets = data["targets"]
        data.update(
            head_out=self.forward(features, proposals, image_shapes, targets)
        )
        return data


# def _RoIHeads(in_channels,  # neck的输出==# backbone的输出
#               num_classes,
#               box_roi_pool=None,
#               box_head=None,
#               box_predictor=None,
...
# min_size=800, max_size=1333, image_mean=None, image_std=None, **kwargs
class MaskRCNNPostprocessor:
    ...
    def __call__(
        ...
        for i, (pred, im_s) in enumerate(zip(result['head_out'][0], result['images'].image_sizes)):
            boxes = pred["boxes"]
...
class SegMetric:
    ...
    def get_metric(self):
        ...
        metrics = {
            result_type: np.mean(
                [result[result_type] for result in self.results]
            )
...
class Model(ConfigModel):
    ...
    # 主干网络
    Backbone = _(_ResNet50, num_classes=26)
    # 颈部
    Neck = _(
    ...
    LRScheduler = _(PiecewiseLR,decay_epochs=[70], values=[0.001, 0.0001], warmup_epoch=5)

    # Transforms = _[ToTensor()
    #     KeepKeys(
    #         "image",
    #         "label",
    #     ),
