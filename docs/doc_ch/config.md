...
|      character_dict_path |    设置字典路径            |  ./ptocr/utils/ppocr_keys_v1.txt  | 如果为空，则默认使用小写字母+数字作为字典 |
...
### Optimizer ([ppocr/optimizer](../../toddleocr/optim))

|         字段             |      用途       |      默认值      |            备注             |
| :---------------------: |:-------------:|:-------------:|   :--------------------:   |
|      name        |     优化器类名     |     Adam      |  目前支持`Momentum`,`Adam`,`RMSProp`, 见[ppocr/optimizer/optimizer.py](../../toddleocr/optim/optimizer.py)  |
|      beta1           | 设置一阶矩估计的指数衰减率 |      0.9      |               \             |
|      beta2           | 设置二阶矩估计的指数衰减率 |     0.999     |               \             |
|      clip_norm           |  所允许的二范数最大值   |               |               \             |
|      **lr**                | 设置学习率decay方式  |       -       |       \  |
|        name    |  学习率decay类名   |    Cosine     | 目前支持`Linear`,`Cosine`,`Step`,`Piecewise`, 见[ppocr/optimizer/learning_rate.py](../../toddleocr/optim/learning_rate.py) |
|        learning_rate      |     基础学习率     |     0.001     |  \        |
|      **regularizer**      |   设置网络正则化方式   |       -       | \        |
|        name      |     正则化类名     |      L2       | 目前支持`L1`,`L2`, 见[ppocr/optimizer/regularizer.py](../../toddleocr/optim/regularizer.py)        |
|        factor      |     正则化系数     |       0.00001        |  \        |


### Architecture ([ppocr/modeling](../../toddleocr/modules))
...
|      **Transform**           |    设置变换方式  |       -       |               目前仅rec类型的算法支持, 具体见[ppocr/modeling/transforms](../../toddleocr/modules/transforms)              |
|        name    |      变换方式类名   |         TPS       | 目前支持`TPS` |
|        num_fiducial      |    TPS控制点数        |       20      |  上下边各十个       |
|        loc_lr      |    定位网络学习率        |       0.1      |  \      |
|        model_name      |    定位网络大小        |       small      |  目前支持`small`,`large`       |
|      **Backbone**      |  设置网络backbone类名        |       -      | 具体见[ppocr/modeling/backbones](../../toddleocr/modules/backbones)        |
...
|      **Neck**      |  设置网络neck        |       -      | 具体见[ppocr/modeling/necks](../../toddleocr/modules/necks)        |
...
|      **Head**      |  设置网络Head        |       -      | 具体见[ppocr/modeling/heads](../../toddleocr/modules/heads)        |
...
### Loss ([ppocr/losses](../../toddleocr/loss))
...
### PostProcess ([ppocr/postprocess](../../toddleocr/postprocess))
...
### Metric ([ppocr/metrics](../../toddleocr/metrics))
...
### Dataset  ([ppocr/data](../../toddleocr/datasets))
|         字段             |            用途            |      默认值        |            备注             |
| :---------------------: |  :---------------------:   | :--------------:  |   :--------------------:   |
|      **dataset**        |         每次迭代返回一个样本          |  -  |  -  |
...
|      transforms        |        对图片和标签进行变换的方法列表         |  [DecodeImage,CTCLabelEncode,RecResizeImg,KeepKeys] |   见[ppocr/data/imaug](../../toddleocr/datasets/imaug)  |
|      **loader**        |        dataloader相关         |  - |   |
