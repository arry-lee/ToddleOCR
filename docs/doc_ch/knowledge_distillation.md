...
蒸馏模型`DistillationModel`类的具体实现代码可以参考[distillation_model.py](../../toddleocr/modules/architectures/distillation_model.py)。
...
关于`CombinedLoss`更加具体的实现可以参考: [combined_loss.py](../../toddleocr/loss/compose.py#L23)。关于`DistillationCTCLoss`等蒸馏损失函数更加具体的实现可以参考[distillation_loss.py](../../toddleocr/loss/distillation.py)。
...
关于`DistillationCTCLabelDecode`更加具体的实现可以参考: [rec_postprocess.py](../../toddleocr/postprocess/rec.py#L128)
...
关于`DistillationMetric`更加具体的实现可以参考: [distillation_metric.py](../../toddleocr/metrics/distillation.py#L24)。
...
关于`DistillationDBPostProcess`更加具体的实现可以参考: [db_postprocess.py](../../toddleocr/postprocess/db.py#L195)
